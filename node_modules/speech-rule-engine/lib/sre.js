...
sre.SystemExternal.WGXpath=sre.Variables.WGXpath;sre.SystemExternal.wgxpath=null;sre.Debugger=function(){this.isActive_=!1;this.outputFunction_=console.info;this.stream_=null};goog.addSingletonGetter(sre.Debugger);sre.Debugger.prototype.init=function(a){a&&this.startDebugFile_(a);this.isActive_=!0};
...
b.mode!==sre.Engine.Mode.HTTP?a.toString():a]},this));var e=c.context||this.activeStore_.context,f=c.action.components;d=[];for(var g=0,h;h=f[g];g++){var k=[],l=h.content||"",m=h.attributes||{},n=!1;h.grammar&&this.processGrammar(e,a,h.grammar);var p=null;if(m.engine){p=sre.Engine.getInstance().dynamicCstr.getComponents();var t=sre.Grammar.parseInput(m.engine);sre.Engine.getInstance().setDynamicCstr(t)}switch(h.type){case sre.SpeechRule.Type.NODE:(l=e.applyQuery(a,l))&&(k=this.evaluateTree_(l));break;
case sre.SpeechRule.Type.MULTI:n=!0;l=e.applySelector(a,l);0<l.length&&(k=this.evaluateNodeList_(e,l,m.sepFunc,e.constructString(a,m.separator),m.ctxtFunc,e.constructString(a,m.context)));break;case sre.SpeechRule.Type.TEXT:var r=m.span;t={};r&&(r=sre.XpathUtil.evalXPath(r,a),r.length&&(t.extid=r[0].getAttribute("extid")));(l=e.constructString(a,l))&&(k=Array.isArray(l)?l.map(function(v){return sre.AuditoryDescription.create({text:v.string,attributes:v.attributes},{adjust:!0})}):[sre.AuditoryDescription.create({text:l,
attributes:t},{adjust:!0})]);break;default:k=[sre.AuditoryDescription.create({text:l})]}k[0]&&!n&&(m.context&&(k[0].context=e.constructString(a,m.context)+(k[0].context||"")),m.annotation&&(k[0].annotation=m.annotation));h.grammar&&sre.Grammar.getInstance().popState();d=d.concat(this.addPersonality_(k,m,n,a));p&&sre.Engine.getInstance().setDynamicCstr(p)}return d};
...
sre.System.prototype.move=function(a){return sre.ProcessorFactory.keypress("move",a)};sre.System.prototype.exit=function(a){var b=a||0;b||sre.Engine.isReady()?sre.SystemExternal.process.exit(b):setTimeout(goog.bind(function(){this.exit(b)},this),100)};sre.Cli=function(){this.system=sre.System.getInstance();this.setup={mode:sre.Engine.Mode.SYNC};this.processors=[];this.dp=new sre.SystemExternal.xmldom.DOMParser({errorHandler:function(a,b){throw new sre.Engine.Error("XML DOM error!");}})};sre.Cli.prototype.set=function(a,b,c){this.setup[a]="undefined"===typeof b?!0:b};sre.Cli.prototype.processor=function(a,b){this.processors.push(b)};
sre.Cli.prototype.enumerate=function(){this.system.setupEngine(this.setup);var a=sre.DynamicCstr.DEFAULT_ORDER.map(function(q){return q.length}),b=function(q,w){a[w]=Math.max.apply(null,Object.keys(q).map(function(x){return x.length}).concat(a[w]))},c=function(q,w){return q+Array(w-q.length+1).join(" ")},d=sre.SpeechRuleEngine.getInstance().enumerate();d=sre.MathCompoundStore.getInstance().enumerate(d);var e=[];b(d,0);for(var f in d){var g=!0,h=d[f];b(h,1);for(var k in h){var l=!0,m=h[k];b(m,2);for(var n in m){var p=
Object.keys(m[n]).sort();if("clearspeak"===n){p=!0;var t=sre.ClearspeakPreferences.getLocalePreferences(d)[f],r;for(r in t)e.push([c(g?f:"",a[0]),c(l?k:"",a[1]),c(p?n:"",a[2]),t[r].join(", ")]),p=l=g=!1}else e.push([c(g?f:"",a[0]),c(l?k:"",a[1]),c(n,a[2]),p.join(", ")]);l=g=!1}}}var v=0,u="";u+=sre.DynamicCstr.DEFAULT_ORDER.slice(0,-1).map(function(q){return c(q,a[v++])}).join(" | ");u+="\n";a.forEach(function(q){u+=Array(q+3).join("=")});u+="========================\n";u+=e.map(function(q){return q.join(" | ")}).join("\n");
console.info(u)};sre.Cli.prototype.execute=function(a){var b=sre.SystemExternal.commander.opts();this.runProcessors_(goog.bind(function(c,d){this.system.processFile(c,d,b.output)},this),a)};sre.Cli.prototype.runProcessors_=function(a,b){try{this.processors.length||this.processors.push("speech"),b&&this.processors.forEach(function(c){a(c,b)})}catch(c){console.error(c.name+": "+c.message),sre.Debugger.getInstance().exit(function(){sre.SystemExternal.process.exit(1)})}};
sre.Cli.prototype.readline=function(){var a=sre.SystemExternal.commander.opts();sre.SystemExternal.process.stdin.setEncoding("utf8");var b=sre.SystemExternal.require("readline").createInterface({input:sre.SystemExternal.process.stdin,output:a.output?sre.SystemExternal.fs.createWriteStream(a.output):sre.SystemExternal.process.stdout}),c="";b.on("line",goog.bind(function(d){c+=d;this.readExpression_(c)&&b.close()},this));b.on("close",goog.bind(function(){this.runProcessors_(goog.bind(function(d,e){b.output.write(sre.ProcessorFactory.output(d,
...
sre.Cli.prototype.commandLine=function(){var a=sre.SystemExternal.commander,b=this.system,c=goog.bind(function(e){return goog.bind(function(f,g){this.set(e,f,g)},this)},this),d=goog.bind(this.processor,this);a.version(b.version).usage("[options] <file ...>").option("-i, --input [name]","Input file [name]. (Deprecated)").option("-o, --output [name]","Output file [name]. Defaults to stdout.").option("-d, --domain [name]","Speech rule set [name]. See --options for details.",c(sre.DynamicCstr.Axis.DOMAIN),
...
"Generate speech output with markup tags.",c("markup"),"none").option("-r, --rate [value]","Base rate [value] for tagged speech output.",c("rate"),"100").option("-p, --speech","Generate speech output (default).",d,"speech").option("-a, --audit","Generate auditory descriptions (JSON format).",d,"description").option("-j, --json","Generate JSON of semantic tree.",d,"json").option("-x, --xml","Generate XML of semantic tree.",d,"semantic").option("-m, --mathml","Generate enriched MathML.",d,"enriched").option("-g, --generate <depth>",
"Include generated speech in enriched MathML (with -m option only).",c("speech"),"none").option("-w, --structure","Include structure attribute in enriched MathML (with -m option only).",c("structure")).option("-P, --pprint","Pretty print output whenever possible.",c("pprint")).option("-f, --rules [name]","Loads a local rule file [name].",c("rules")).option("-C, --prune [branch]","Prune trie [branch] for clean reload.",c("prune")).option("-v, --verbose","Verbose mode.").option("-l, --log [name]","Log file [name].").option("--opt",
"List engine setup options.").on("option:opt",goog.bind(function(){this.enumerate();sre.System.getInstance().exit(0)},this)).parse(sre.SystemExternal.process.argv);this.system.setupEngine(this.setup);b=a.opts();b.verbose&&sre.Debugger.getInstance().init(b.log);b.input&&this.execute(b.input);a.args.length?a.args.forEach(goog.bind(this.execute,this)):this.readline();sre.Debugger.getInstance().exit(function(){sre.System.getInstance().exit(0)})};
...
module.exports.engineSetup=sre.System.getInstance().engineSetup;module.exports.exit=sre.System.getInstance().exit;global&&global.SRE_JSON_PATH&&sre.System.setAsync();
